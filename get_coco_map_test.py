...
#   指向了验证集标签与图片路径
# -------------------------------------------------------#
cocoGt_path = ROOT / 'coco_dataset/test_annotations/image_info_test2017.json'
dataset_img_path = ROOT / 'coco_dataset/test2017'

# -------------------------------------------------------#
#   结果输出的文件夹，默认为map_out
# -------------------------------------------------------#
temp_save_path = ROOT / 'map_out_test/coco_eval'
...
class mAP_YOLO(YOLO):
    ...
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image_id, image, results, clsid2catid):  # 方法重载
        # ---------------------------------------------------#
        #   计算输入图片的高和宽
        # ---------------------------------------------------#
...
if __name__ == "__main__":
    ...
    clsid2catid = cocoGt.getCatIds()  # 获取类别 id
