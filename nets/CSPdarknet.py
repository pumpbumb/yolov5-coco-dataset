...
class Focus(nn.Module):
    ...
    def forward(self, x):
        return self.conv(  # torch.cat 就是在 focus , 然后再经过一个卷积层
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])  # shortcut 送到瓶颈结构
        # 上一行就是 self.m = Bottleneck(c_, c_, shortcut, g, e=1.0)
        # self.m 属性的 e 不应该是 0.5嘛，怎么是 1.0
